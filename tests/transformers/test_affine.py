...
four_by_four = [[1, 1, 2, 2], [1, 1, 2, 2], [3, 3, 4, 4], [3, 3, 4, 4]]
...
scale_double = [[2, 0, 0], [0, 2, 0], [0, 0, 1]]

translate = [[1, 0, 5], [0, 1, 10], [0, 0, 1]]
...
affine_xform_img_args = [
    ...
    (two_by_two, [[-1, 0, 1], [0, 1, 0], [0, 0, 1]], [[2, 1], [4, 3]]),
    (four_by_four, [[-1, 0, 4], [0, 1, 0], [0, 0, 1]], [[2, 2, 1, 1], [2, 2, 1, 1], [4, 4, 3, 3], [4, 4, 3, 3]]),
]
...
def test_affine_transform_image(src_img, affine, exp_img):
    ...
    output_accessor = ImageAccessor(
        x=(0, exp_nd.shape[1]), y=(0, exp_nd.shape[0]), metadata=meta, interpolation="nearest"
    )
...
mirror = [[-1, 0, IMG_SIZE - 1], [0, 1, 0], [0, 0, 1]]
...
def test_affine_transform_access(src_xy, affine, expected_xy):
    ...
    xformer = AffineTransformer(np.array(affine), request_margin=0)
