...
result = pipeline(file_name="./data/Koala.jpg", accessor=ImageAccessor(spacing=2.0, coordinate_spacing=1.0), image_spacing=1.0)
print(result.image.shape)

# Now something more sophisticated: We specify the coordinates, in a different spacing than what we want to read...
result_4micron = pipeline(file_name="./data/Koala.jpg", accessor=ImageAccessor(spacing=2.0, coordinate_spacing=4.0, x=(200, 300), y=(200, 300)), image_spacing=1.0)
print(result_4micron.image.shape)

# ...which is the same as:
result_2micron = pipeline(file_name="./data/Koala.jpg", accessor=ImageAccessor(spacing=2.0, coordinate_spacing=2.0, x=(400, 600), y=(400, 600)), image_spacing=1.0)
assert np.allclose(result_2micron.image, result_4micron.image)

# ...and:
result_1micron = pipeline(file_name="./data/Koala.jpg", accessor=ImageAccessor(spacing=2.0, coordinate_spacing=1.0, x=(800, 1200), y=(800, 1200)), image_spacing=1.0)
...
result = pipeline(file_name="./data/Koala.jpg", accessor=ImageAccessor())
print(result.image.shape)

# But we can now also specify image dimension in fractions of the total image size.
result = pipeline(file_name="./data/Koala.jpg", accessor=ImageAccessor(x=(0, 0.5), y=(0, 0.5)))
print(result.image.shape)
...
result = pipeline(file_name="./data/Koala.jpg", accessor=ImageAccessor(x=(0, 0.5), y=(0, 0.5), spacing=2.0), image_spacing=1.0)
print(result.image.shape)
